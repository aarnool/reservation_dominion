...
async def test_get_all_reservations_success(
    ...
    resource = Resources(name="Reserva Admin Test", capacity=10)
...
async def test_approve_reservation_success(
    admin_client: AsyncClient, db_session: AsyncSession
):
    """Prueba aprobar una reserva."""
    resource = Resources(name="Reserva Aprobar", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
...
async def test_create_reservation_success(
    ...
    resource = Resources(name="Sala Test", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
...
async def test_update_reservation_success(
    ...
    resource = Resources(name="Sala Update", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
    ...
    response = Reservation(
        ...
        resource_id=resource.id,
        title="Test Update",
        start_time=datetime(2026, 8, 14, 10, 0, tzinfo=UTC),
        end_time=datetime(2026, 8, 14, 12, 0, tzinfo=UTC),
    )
    ...
    resp2 = await user_client.patch(
        f"/reservations/{res_id}", json={"title": "Titulo Actualizado"}
    )
...
async def test_cancel_reservation_success(
    ...
    resource = Resources(name="Sala Cancel", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
    ...
    resp1 = Reservation(
        ...
        resource_id=resource.id,
        title="Test Cancel",
        start_time=datetime(2026, 8, 17, 10, 0, tzinfo=UTC),
        end_time=datetime(2026, 8, 17, 12, 0, tzinfo=UTC),
    )
