...
async def test_approve_reservation_already_confirmed(
...
async def test_approve_reservation_cancelled(
    admin_client: AsyncClient, db_session: AsyncSession
):

    resource = Resources(name="Reserva Cancelada", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
...
async def test_create_reservation_start_after_end(
    user_client: AsyncClient, db_session: AsyncSession
):

    resource = Resources(name="Sala Error", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
...
async def test_update_reservation_start_after_end(
    user_client: AsyncClient, db_session: AsyncSession
):

    resource = Resources(name="Sala Update Error", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
    ...
    resp1 = Reservation(
        ...
        start_time=datetime(2026, 8, 16, 10, 0, tzinfo=UTC),
        end_time=datetime(2026, 8, 16, 12, 0, tzinfo=UTC),
    )
    ...
    resp2 = await user_client.patch(
        f"/reservations/{res_id}",
        json={"start_time": "2026-08-16T14:00:00Z", "end_time": "2026-08-16T12:00:00Z"},
    )
    assert resp2.status_code == 409


# Verificar que un usuario quiere actualizar una reserva con un recurso que no existe, se devuelva un error 404
async def test_update_reservation_invalid_resource(
    user_client: AsyncClient, db_session: AsyncSession
):

    resource = Resources(name="Sala Update Invalid", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
    ...
    resp2 = await user_client.patch(
        f"/reservations/{res_id}", json={"resource_id": 99999}
    )
...
async def test_cancel_reservation_already_cancelled(
    ...
    resource = Resources(name="Sala Double Cancel", capacity=10)
    db_session.add(resource)
    await db_session.commit()
    await db_session.refresh(resource)
    ...
    resp1 = Reservation(
        ...
        start_time=datetime(2026, 8, 18, 10, 0, tzinfo=UTC),
        end_time=datetime(2026, 8, 18, 12, 0, tzinfo=UTC),
    )
    db_session.add(resp1)
    await db_session.commit()
    await db_session.refresh(resp1)
