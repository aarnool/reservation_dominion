...
async def test_create_resource_invalid_capacity(admin_client: AsyncClient):
    ...
    response = await admin_client.post(
        "/resources/", json={"name": "Sala Invalida", "capacity": 0}
    )
...
async def test_update_resource_duplicate_name(
    admin_client: AsyncClient, db_session: AsyncSession
):

    res1 = Resources(name="Sala A", capacity=10)
    db_session.add(res1)
    await db_session.commit()
    await db_session.refresh(res1)

    res2 = Resources(name="Sala B", capacity=15)
    db_session.add(res2)
    await db_session.commit()
    ...
    response = await admin_client.patch(
        f"/resources/{res2_id}", json={"name": "Sala A"}
    )
    assert response.status_code == 409
    assert (
        response.json()["detail"]
        == "El nombre del recurso ya está en uso por otro recurso."
    )


# Prueba eliminar un recurso que tiene reservas asociadas y espera un 400 Bad Request.
async def test_delete_resource_with_reservations(
    admin_client: AsyncClient, db_session: AsyncSession
):

    resource = Resources(name="Sala con Reserva", capacity=10)
